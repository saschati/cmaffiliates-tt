...
frontend-test: ## Run test
	$(docker_compose_bin) -f docker-compose.yml $(docker_compose_override) run --rm frontend-node-cli yarn test --watchAll=false
...
frontend-analyze-refresh: ## Refresh build for analyze
	$(docker_compose_bin) -f docker-compose.yml $(docker_compose_override) exec frontend-analyze yarn build
